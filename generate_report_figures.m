function outputFiles = generate_report_figures(cfg)
...
plot_tx_constellation(outputFiles{1});
plot_ber(berData.result, outputFiles{2});
plot_rx_constellation(functional.constellationLinks{1}, ...
    functional.constellationEbNoDb(1), '低信噪比', outputFiles{3});
plot_rx_constellation(functional.constellationLinks{end}, ...
    functional.constellationEbNoDb(end), '高信噪比', outputFiles{4});
plot_key_nodes(functional.bits, functional.functionalLink, outputFiles{5});
plot_spectrum(functional.functionalLink.txWaveform, outputFiles{6});
end

function plot_tx_constellation(filePath)
pairs = logical([0 0; 0 1; 1 1; 1 0]);
symbols = qpsk_modulate(reshape(pairs.', [], 1));
fig = report_figure([7.2 7.1]);
...
function plot_rx_constellation(link, ebnoDb, levelText, filePath)
fig = report_figure([7.2 7.1]);
...
function plot_key_nodes(bits, link, filePath)
fig = report_figure([7 6.1]);
...
function plot_spectrum(waveform, filePath)
...
function fig = report_figure(sizeInches)
...
function apply_report_style(fig)
axesHandles = findall(fig, 'Type', 'axes');
set(axesHandles, 'FontName', 'Microsoft YaHei', 'FontSize', 10.5, ...
    'LineWidth', 0.9, 'Box', 'on', 'Color', 'w', ...
    'GridAlpha', 0.18, 'MinorGridAlpha', 0.10);
for k = 1:numel(axesHandles)
    axesHandles(k).Title.FontName = 'Microsoft YaHei';
    axesHandles(k).Title.FontSize = 13;
    axesHandles(k).Title.FontWeight = 'bold';
    axesHandles(k).XLabel.FontName = 'Microsoft YaHei';
    axesHandles(k).XLabel.FontSize = 11.5;
    axesHandles(k).YLabel.FontName = 'Microsoft YaHei';
    axesHandles(k).YLabel.FontSize = 11.5;
end
legendHandles = findall(fig, 'Type', 'legend');
set(legendHandles, 'FontName', 'Microsoft YaHei', 'FontSize', 9.5, ...
    'Box', 'on');
textHandles = findall(fig, 'Type', 'text');
set(textHandles, 'FontName', 'Microsoft YaHei');
end
